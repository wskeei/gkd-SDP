...
class AppBlockerVmDisableQuotaTest {
    private val zoneId = ZoneId.systemDefault()

    @Test
    fun disableTransitionAllowedThenBlockedWithinSameDay() {
        val now = LocalDateTime.of(2026, 2, 21, 12, 0).atZone(zoneId).toInstant().toEpochMilli()
        val dayStart = LocalDateTime.of(2026, 2, 21, 0, 0).atZone(zoneId).toInstant().toEpochMilli()

        assertTrue(AppBlockerVm.shouldConsumeDisableQuota(currentEnabled = true, requestedEnabled = false))
    ...
    fun dayRolloverAllowsDisableAgain() {
        val dayStart = LocalDateTime.of(2026, 2, 21, 0, 0).atZone(zoneId).toInstant().toEpochMilli()
        val nextDay = LocalDateTime.of(2026, 2, 22, 12, 0).atZone(zoneId).toInstant().toEpochMilli()
