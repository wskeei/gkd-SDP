...
class RoomSchema33Test(unittest.TestCase):
    def load(self, version: int):
        ...
        # Temporary Draft-PR bootstrap: the first CI run must reach the Room
        # processor so its generated schema can be downloaded. This branch is
        # removed as soon as schema 33 is committed.
        if version == 33 and not path.is_file():
            self.skipTest(f"schema {version} will be generated by the CI processor")
