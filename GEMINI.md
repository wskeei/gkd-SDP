...
5.  ✅ **锁定功能**：支持三层锁定机制，锁定后无法关闭、删除或修改规则/组，且 UI 进入只读状态。
6.  ✅ **规则组优化**：支持在规则组内直接添加网址及批量导入功能。
...
11. ✅ **强制锁定**：实现锁定状态下的只读 UI，封堵通过保存操作绕过时间限制的漏洞。
...
| 2026-01-22 | ✅ 完成功能：专注模式快速启动小组件 (Focus Quick Start Widget)，支持桌面一键开启专注 |
| 2026-01-25 | 🔧 网址拦截优化：支持规则组直接添加网址及批量导入；恢复独立规则的时间模式设置；全面封堵锁定状态下的修改漏洞 |
